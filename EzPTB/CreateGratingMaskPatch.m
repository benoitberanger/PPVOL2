function [patch] = CreateGratingMaskPatch(diameter, orientation, frequency, phase, contrast, noise, falloff, binary)
...
bbox = floor([diameter(2),diameter(2)]/2)*2;
...
patch(:,:,1) = 0.5*(contrast*patch(:,:,1)+1);
patch(:,:,1) = min(max(patch(:,:,1), 0), 1);

lambda = log(1/0.01-1)*2/falloff;
patch(:,:,2) = sigmoid(lambda*(sqrt(x.^2+y.^2)-(diameter(1)+falloff)/2), [0,1]);
patch(:,:,2) = min(patch(:,:,2), sigmoid(lambda*(sqrt(x.^2+y.^2)-(diameter(2)-falloff)/2), [1,0]));
